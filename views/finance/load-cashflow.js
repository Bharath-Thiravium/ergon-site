async function loadCashFlow() {
    try {
        ...
        if (!prefix) {
            console.log('No prefix selected, skipping cashflow load');
            return;
        }
        ...
        if (result.success && result.data) {
            ...
            if (expectedInflowEl) {
                expectedInflowEl.textContent = '₹' + parseFloat(data.expected_inflow).toLocaleString('en-IN', {maximumFractionDigits: 2});
            }
            
            if (poCommitmentsEl) {
                poCommitmentsEl.textContent = '₹' + parseFloat(data.po_commitments).toLocaleString('en-IN', {maximumFractionDigits: 2});
            }
...
if (prefixInput) {
    ...
    prefixInput.addEventListener('input', function() {
        ...
        this.prefixTimeout = setTimeout(loadCashFlow, 500);
    });
}
